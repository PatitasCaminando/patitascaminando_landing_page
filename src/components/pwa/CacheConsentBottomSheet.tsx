...
export const CacheConsentBottomSheet = () => {
  ...
  // No renderizar en el servidor ni si ya fue aceptado
  if (hasAcceptedCache === null || hasAcceptedCache) {
    return null;
  }
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 bg-[#153970]/10 z-40 pointer-events-none transition-opacity duration-300 ${isPolicyModalOpen ? 'opacity-0' : 'opacity-100'}`}></div>
      
      {/* Bottom Sheet Principal */}
      <div className={`fixed bottom-0 left-0 right-0 z-50 bg-[#FFF7EA] rounded-t-3xl shadow-[0_-10px_40px_rgba(21,57,112,0.1)] border-t border-[#F1D9BD] p-6 pb-8 md:p-8 md:pb-10 transition-transform duration-400 ease-in-out ${isPolicyModalOpen ? 'translate-y-full' : 'translate-y-0 animate-[slideUp_0.4s_ease-out_forwards]'}`}>
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-start md:items-center gap-6 relative z-20">
          ...
          <div className="flex-1 w-full sm:pl-[10.5rem] md:pl-[13.5rem] lg:pl-[15.5rem] relative z-20">
            <div className="flex items-start md:items-center justify-between md:justify-start gap-2 mb-3">
                <h3 className="text-lg md:text-xl font-extrabold text-[#153970] leading-tight">
            ...
            <Button variant="primary" onClick={acceptCacheConsent} className="w-full sm:w-auto justify-center shadow-md flex items-center gap-1.5">
      ...
      {/* Modal Secundario de Política de Caché */}
      {isPolicyModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-[#153970]/40 backdrop-blur-sm animate-[fadeIn_0.3s_ease-out]">
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-lg w-full shadow-2xl relative animate-[scaleIn_0.3s_ease-out]">
            <button 
              onClick={() => setIsPolicyModalOpen(false)}
              className="absolute top-5 right-5 bg-gray-50 hover:bg-gray-100 text-gray-400 hover:text-gray-600 p-2 rounded-full transition-colors"
            >
              <X size={20} />
            </button>
            
            <div className="flex items-center gap-3 mb-6">
              <div className="bg-[#EBF3FF] p-2.5 rounded-2xl text-[#153970]">
                <Info size={24} />
              </div>
              <h3 className="text-xl font-extrabold text-[#153970]">Política de uso de caché</h3>
            </div>
            
            <div className="text-[#5F6B70] text-sm leading-relaxed space-y-4">
              <p>
